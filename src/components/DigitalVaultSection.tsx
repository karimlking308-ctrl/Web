...
import { TelegramCommunityBanner } from './TelegramCommunityBanner';

export const DigitalVaultSection: React.FC = () => {
  // Category Filter
  ...
  const handleDownloadProduct = async (productId: string, specificFormat?: string) => {
    ...
    try {
      if (productId === 'product-telegram-miniapp') {
        await generateTelegramMiniAppZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Telegram Mini-App & TON Clicker Game source code package downloaded!');
      } else if (productId === 'product-whatsapp-ai-leadgen') {
        await generateWhatsAppAILeadGenZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('WhatsApp AI Agent Lead Generator project package downloaded!');
      } else if (productId === 'product-solana-sniper-bot') {
        await generateSolanaSniperBotZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Solana Token Sniper & Launch Bot Kit downloaded!');
      } else if (productId === 'product-n8n-workflows') {
        if (specificFormat === 'json') {
          generateN8nWorkflowsJSON();
          setSuccessMessage('n8n AI Workflows JSON package downloaded!');
        } else {
          await generateN8nWorkflowsZIP('SOLPUMP-FREE-OPEN-ACCESS');
          setSuccessMessage('n8n Workflows Full Archive (.ZIP) package downloaded!');
        }
      } else if (productId === 'product-webhook-boilerplates') {
        await generateWebhookBoilerplateZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Webhook & API Microservice Boilerplates downloaded!');
      } else if (productId === 'product-solana-buybot') {
        await generateTelegramBuyBotZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Solana Telegram Buy-Bot Engine source code downloaded!');
      } else if (productId === 'product-prompt-vault') {
        if (specificFormat === 'json') {
          generatePromptVaultJSON();
          setSuccessMessage('AI & Web3 Prompt Vault JSON database downloaded!');
        } else {
          generatePromptVaultMarkdown();
          setSuccessMessage('AI & Web3 Prompt Vault Markdown handbook downloaded!');
        }
      } else if (productId === 'product-react-boilerplate') {
        await generateReactBoilerplateZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('React 19 + Tailwind CSS SaaS Starter Codebase downloaded!');
      } else if (productId === 'product-solana-bulk-sender') {
        await generateBulkSenderScriptZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Solana Bulk Sender Script package downloaded!');
      } else if (productId === 'product-telegram-broadcast-bot') {
        await generateTelegramBroadcastScriptZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Telegram Mass Broadcaster Bot package downloaded!');
      } else if (productId === 'product-ai-content-batch-generator') {
        await generateAIContentBatchScriptZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('AI Content Batch Generator package downloaded!');
      } else if (productId === 'product-rust-tx-dispatcher') {
        await generateRustTxDispatcherScriptZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Rust Transaction Dispatcher package downloaded!');
      } else if (productId === 'product-solana-toolkit') {
        await generateSolanaToolkitZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Solana Anchor 0.30 & Token-2022 Toolkit downloaded!');
      } else if (productId === 'master-bundle') {
        await generateMasterBundleZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Master All-In-One Digital Asset Archive (.ZIP) package downloaded!');
      } else {
        await generateMasterBundleZIP('SOLPUMP-FREE-OPEN-ACCESS');
        setSuccessMessage('Digital Asset Archive (.ZIP) package downloaded!');
      }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold uppercase tracking-wider mb-3">
            ...
            <span>100% Free Open Access Resource Hub</span>
          </div>
          <h2 className="text-3xl sm:text-5xl font-extrabold text-white tracking-tight mb-4">
            Digital Vault &amp; Asset Downloads
          </h2>
          <p className="text-slate-400 text-sm sm:text-base leading-relaxed max-w-2xl mx-auto">
            Instant 1-click access to ready-to-import n8n AI agent workflows, backend webhooks, Telegram buy-bots, React 19 source code, and 1,500+ prompt databases.
          </p>
        </div>

        {/* ========================================================= */}
        {/* VAULT ACCESS STATUS BAR - FREE COMMUNITY ACCESS */}
        {/* ========================================================= */}
        <div className="mb-10 p-5 sm:p-6 rounded-3xl bg-gradient-to-r from-slate-900 via-[#0d1322] to-slate-900 border border-emerald-500/30 shadow-xl shadow-emerald-500/5">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-center gap-3.5">
              <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/40 text-emerald-400 flex items-center justify-center shrink-0 shadow-lg shadow-emerald-500/10">
              ...
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono-code font-extrabold bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                    <span>FREE COMMUNITY ACCESS</span>
                  </span>
                </div>
                <p className="text-xs text-slate-300 mt-1">
                  100% Open Access Resource Hub — No payment required, no locks, no license key needed. Download any toolkit with 1-click.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2.5 w-full md:w-auto">
              <a
                ...
                className="px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-mono-code text-cyan-300 flex items-center gap-1.5 transition-colors cursor-pointer"
              >
              ...
              >
                ...
                <span>Download All Products (.ZIP)</span>
              </button>
            </div>
          </div>
        ...
        {/* ========================================================= */}
        <TelegramCommunityBanner compact />
        ...
        <div className="flex items-center justify-center gap-2 mb-10 overflow-x-auto pb-2 scrollbar-none">
          {[
            { id: 'all', label: 'All Free Assets', count: DIGITAL_PRODUCTS.length },
        ...
        {/* ========================================================= */}
        {/* DIGITAL PRODUCTS GRID (6 TOTAL) - 100% DIRECT DOWNLOAD */}
        {/* ========================================================= */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProducts.map((prod) => {
            ...
            return (
              <div
                key={prod.id}
                className="rounded-3xl p-6 sm:p-7 flex flex-col justify-between transition-all duration-200 border relative bg-[#0c111e] border-slate-800 hover:border-emerald-500/40 shadow-xl"
              >
                {/* Top Badge & Category indicator */}
                <div className="flex items-center justify-between gap-2 mb-3">
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-mono-code uppercase font-bold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h4 className="text-lg font-extrabold text-white">
                Master All-In-One Free Creator Bundle (.ZIP)
              </h4>
              <p className="text-xs text-slate-300 mt-1 max-w-xl">
                Download all products (n8n workflows, webhooks, Telegram buy-bot, 1,500+ prompts, React 19 boilerplate, and Anchor toolkits) in a single organized 100% free master archive.
              </p>
            </div>
          </div>
