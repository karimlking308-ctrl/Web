...
export const LengthConverterTool: React.FC = () => {
  ...
  const units: Record<string, { name: string; nameAr: string; toMeters: number; symbol: string }> = {
    m: { name: 'Meters', nameAr: 'متر', toMeters: 1, symbol: 'm' },
    km: { name: 'Kilometers', nameAr: 'كيلومتر', toMeters: 1000, symbol: 'km' },
    cm: { name: 'Centimeters', nameAr: 'سنتيمتر', toMeters: 0.01, symbol: 'cm' },
    mm: { name: 'Millimeters', nameAr: 'مليمتر', toMeters: 0.001, symbol: 'mm' },
    mi: { name: 'Miles', nameAr: 'ميل', toMeters: 1609.344, symbol: 'mi' },
    yd: { name: 'Yards', nameAr: 'يارده', toMeters: 0.9144, symbol: 'yd' },
    ft: { name: 'Feet', nameAr: 'قدم', toMeters: 0.3048, symbol: 'ft' },
    in: { name: 'Inches', nameAr: 'بوصة / إنش', toMeters: 0.0254, symbol: 'in' },
    nmi: { name: 'Nautical Miles', nameAr: 'ميل بحري', toMeters: 1852, symbol: 'NM' },
  };
  ...
  // Conversion
  const meters = (value || 0) * units[fromUnit].toMeters;
  const converted = meters / units[toUnit].toMeters;
    ...
    <div className="space-y-6">
      ...
      <div className="p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
        <div className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-3 flex items-center gap-1.5">
          ...
          {isAr ? `تحويل ${value} ${units[fromUnit].nameAr} إلى جميع وحدات الطول:` : `All Length Conversions for ${value} ${units[fromUnit].name}:`}
        </div>
